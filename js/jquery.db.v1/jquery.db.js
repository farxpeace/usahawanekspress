	...
	jQuery.extend({
		db: {
			...
			open_db: function (settings) {
				this._db = openDatabase(settings.shortName, settings.version, settings.name, settings.maxSize);
			},
			create_table: function (settings, ifExists, resultsfn, errorfn) {
				...
				return this.query(query, resultsfn, errorfn);
			},
			normalize_results: function (results) {
				...
				if (obj.num_rows > 0) {
					for (var i=0; i<results.rows.length; i++) {
						obj.results.push(results.rows.item(i));
					}
				}
				
				return obj;
			},
			query: function (query, resultsfn, errorfn) {
				...
				var fn = function (transaction) {
					transaction.executeSql(query, [], function (transaction, results) {
						if (resultsfn) resultsfn(this.normalize_results(results), transaction, query);
			...
			get: function (table, resultfn, errorfn) {
				...
				return this.query(query, resultfn, errorfn);
			},
